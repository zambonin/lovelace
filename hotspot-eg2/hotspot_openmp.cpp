...
void single_iteration(FLOAT *result, FLOAT *temp, FLOAT *power, int row,
  ...
  int chunk,
      num_chunks = row * col / (BLOCK_SIZE_C * BLOCK_SIZE_R),
      chunks_in_col = col / BLOCK_SIZE_C, chunks_in_row = row / BLOCK_SIZE_R;
...
#pragma omp parallel for shared(power, temp, result) private(                  \
    r, c, delta) firstprivate(row, col) schedule(static) private(              \
        chunk) firstprivate(num_chunks, chunks_in_row, chunks_in_col)
#endif
  ...
  int r_start = BLOCK_SIZE_R * (chunk / chunks_in_col),
    ...
    if (r_start == 0 || c_start == 0 || r_end == row || c_end == col) {
      for (r = r_start; r < r_end; r += BLOCK_SIZE_R) {
        for (c = c_start; c < c_end; c += BLOCK_SIZE_C) {
          /* Corner 1 */
          if ((r == 0) && (c == 0)) {
