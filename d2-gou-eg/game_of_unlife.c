...
void evolve(void *u, uint32_t w, uint32_t h) {
  ...
  int chunk,
      num_chunks = w * h / (STEP * STEP),
      chunks_in_row = h / STEP,
      chunks_in_col = w / STEP;

#pragma omp parallel for shared(u, univ, new) \
	firstprivate(w, h, num_chunks, chunks_in_row, chunks_in_col) \
	private(x, x1, y, y1, chunk) schedule(static)
  for (chunk = 0; chunk < num_chunks; chunk++) {
    int r_start = STEP * (chunk / chunks_in_col),
        c_start = STEP * (chunk % chunks_in_row),
        r_end = r_start + STEP > h ? h : r_start + STEP,
        c_end = c_start + STEP > w ? w : c_start + STEP,
 				r, c;
